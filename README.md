...
AdventureTube transforms your existing YouTube content into an interactive, location-based storytelling experience without requiring you to create new content. It seamlessly integrates your videos with a map, allowing you to structure and present your adventures in an engaging, geo-tagged format.

- **Transform YouTube Videos into Location-Based Stories** –  AdventureTube enhances your existing videos.
- **Content Download & Management** – Import and organize your YouTube videos within the app.
- **Content Segmentation** – Divide videos into meaningful chapters(same concept of a chapter in YouTube) that highlight key moments.
- **Geo-Mapped Chapters** – Each chapter is linked to a real-world location, enabling immersive exploration.
- **Interactive Mapping & Storytelling** – Drag, adjust, and manage your story directly on the map. (check the Figma prototype)
- **Smart Content Repackaging** – Your single long content will be re-packaged with multiple short chapters each chapter works as an attraction for the user independently which is the reason the content value will significantly increase. 
...
- A Google Cloud account with enabled APIs (Google Sign-In, Places API, YouTube Data API). API keys are required for each of these services and can be requested from [Google Cloud Console](https://console.cloud.google.com/apis/credentials):
  ...
  - **YouTube Data API Key**: Needed for fetching and managing YouTube content. Ensure these API keys are properly configured and restricted for security.
